%Same as ex3 but split test and train
...
%% ================ Part 4: Show some erros ================

fail = ~(pred == y_test);
X_fail = X_test(fail,:);

for i = 1:size(X_fail,1)
    % Display 
    fprintf('\nDisplaying Example Image\n');
    displayData(X_fail(i, :));

    pred = predictOneVsAll(all_theta, X_fail(i,:));
    fprintf('\nOne vs All Logistic Classifier Prediction: digit %d\n', mod(pred, 10));
